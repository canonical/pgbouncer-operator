...
class TestCharm(unittest.TestCase):
    ...
    def test_on_install(
        ...
        _mkdir.assert_any_call(PGB_DIR, 0o740)
        _chown.assert_any_call(PGB_DIR, 1100, 120)

        for service_id in self.charm.service_ids:
            _mkdir.assert_any_call(f"{PGB_DIR}/instance_{service_id}", 0o740)
        ...
        _render_file.assert_any_call(USERLIST_PATH, initial_userlist, 0o740)
    ...
    def test_render_pgb_config(self, _render, _reload):
        ...
        _render.assert_called_with(INI_PATH, test_config.render(), 0o740)
        ...
        _render.assert_called_with(INI_PATH, reload_config.render(), 0o740)
        _reload.assert_called()

        self.charm._render_pgb_config(reload_config, config_path="/test/path")
        _render.assert_called_with("/test/path", reload_config.render(), 0o740)
    ...
    def test_render_service_configs(self, _render, _reload):
        ...
        _render.assert_any_call(INI_PATH, cfg_list[0], 0o740)
        _render.assert_any_call(f"{PGB_DIR}/instance_0/pgbouncer.ini", cfg_list[1], 0o740)
        _render.assert_any_call(f"{PGB_DIR}/instance_1/pgbouncer.ini", cfg_list[2], 0o740)
    ...
    def test_render_userlist(self, _render, _reload):
        ...
        _render.assert_called_with(USERLIST_PATH, pgb.generate_userlist(test_users), 0o740)
        _reload.assert_not_called()

        reload_users = {"reload_user": "reload_pass"}
        self.charm._render_userlist(reload_users, reload_pgbouncer=True)
        _render.assert_called_with(USERLIST_PATH, pgb.generate_userlist(reload_users), 0o740)
        _reload.assert_called()
