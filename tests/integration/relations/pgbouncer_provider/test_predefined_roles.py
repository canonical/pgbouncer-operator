...
def test_deploy(juju: jubilant.Juju, charm_noble, predefined_roles_combinations) -> None:
    ...
    for combination in combinations:
        ...
        # Deploy the PgBouncer charm for each combination of predefined roles.
        pgbouncer_app_name = f"{PGB}{suffix}"
        if pgbouncer_app_name not in juju.status().apps:
            logger.info("Deploying PgBouncer charm")
            juju.deploy(charm_noble, app=pgbouncer_app_name, base="ubuntu@24.04")
        ...
        # Relate the PgBouncer charm to the database charm and the data integrator
        # charm to the PgBouncer charm.
        existing_relations = relations(juju, PG, pgbouncer_app_name)
        if not existing_relations:
            logger.info("Adding relation between charms")
            juju.integrate(PG, pgbouncer_app_name)
        existing_relations = relations(juju, pgbouncer_app_name, data_integrator_app_name)
        if not existing_relations:
            logger.info("Adding relation between charms")
            juju.integrate(pgbouncer_app_name, data_integrator_app_name)
